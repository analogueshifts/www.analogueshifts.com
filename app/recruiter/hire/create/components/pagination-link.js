...
export default function PaginationLink({
    ...
    return (
        <Link
            ...
            <div
                className={`large:w-6  tablet:w-3 tablet:p-[1px] tablet:h-3 large:h-6 p-0.5 rounded-full border border-greenPea flex justify-center items-center ${checkCurrentPath() || formFilled()
                    ? 'bg-greenPea'
                    : 'bg-transparent'
                ...
                {!formFilled() && checkCurrentPath() && (
                    <div
                        className={` tablet:w-1 tablet:h-1 rounded-full bg-platinum ${inModal ? 'w-2 h-2' : 'w-2.5 h-2.5'
                            }`}></div>
                )}
            </div>
            <p
                className={`text-black  tablet:leading-none large:text-lg font-normal leading-8 ${inModal
