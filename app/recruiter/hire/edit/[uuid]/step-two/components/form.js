...
import RenderScreeningQuestion from '../../../../create/step-two/components/RenderScreeningQuestion'
...
export default function Form() {
    ...
    useEffect(() => {
        if (
            skillTracker !== '' &&
            !editJob.stepTwo.documents?.includes(skillTracker)
        ) {
            setEditJob(prev => {
                return {
                    ...prev,
    ...
    return (
        <form
            ...
            />
            {editJob.stepTwo?.directApply !== 'easy' && (
                <div
                    className={`w-full grid grid-cols-2 tablet:gap-5 tablet:grid-cols-1 tablet:pb-6 items-start pb-10`}>
                    <div className="col-span-1 flex flex-col tablet:gap-2.5 gap-5">
                        <h3 className="large:text-base text-sm font-semibold text-tremor-brand-boulder950 max-w-[80%] tablet:max-w-full w-max">
            ...
            {editJob.stepTwo?.directApply === 'easy' && (
                <>
                    <div
                        ...
                        <div className={`col-span-1`}>
                            {editJob.stepTwo.documents?.length > 0 && (
                                <ActionList
                                    list={editJob.stepTwo.documents}
                                    setList={list => {
                                        setEditJob(prev => {
                                            return {
                                                ...prev,
